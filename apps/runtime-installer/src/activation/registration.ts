  ...
  try {
    ...
    if (err instanceof HttpError) {
      throw new RegistrationError(
        httpErrorMessage(err.code, err.message),
        err.code,
      );
    }
    throw err;
...
function httpErrorMessage(code: string, message: string): string {
  switch (code) {
    case 'TOKEN_NOT_FOUND':           return 'Activation token not found. Please check the token and try again.';
    case 'TOKEN_EXPIRED':             return 'Activation token has expired. Request a new token from the console.';
    case 'TOKEN_USED':                return 'Activation token has already been used. Each token can only be used once.';
    case 'MACHINE_ALREADY_REGISTERED':return 'This machine is already registered. Use the console to manage the existing agent.';
    case 'VALIDATION_ERROR':          return `Validation error: ${message}`;
