  ...
  return (
    <div className={cn('space-y-1', className)}>
      <div className="flex h-1.5 w-full rounded-full overflow-hidden bg-slate-100">
        {synced > 0 && (
          <div className="bg-emerald-500 h-full" style={{ width: `${(synced / total) * 100}%` }} />
        )}
        {skipped > 0 && (
          <div className="bg-slate-300  h-full" style={{ width: `${(skipped / total) * 100}%` }} />
        )}
        {failed > 0 && (
          <div className="bg-rose-400   h-full" style={{ width: `${(failed / total) * 100}%` }} />
        )}
      </div>
