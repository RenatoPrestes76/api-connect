...
import {
  canonicalAckPayload,
  canonicalPendingPollPayload,
} from '../../modules/message-delivery/message-signature.js';
...
/** Creates a QUEUED Job via the already-built job-orchestration module, to link a message to for EXECUTING->COMPLETED/FAILED sync tests. */
export async function createJob(
  baseUrl: string,
  auth: Record<string, string>,
  input: { organizationId: string; runtimeId: string }
): Promise<string> {
  const { body } = await post<{ job: { id: string } }>(
    baseUrl,
    '/jobs',
    { organizationId: input.organizationId, runtimeId: input.runtimeId, command: 'PING' },
    auth
  );
  return body.job.id;
}
...
export function signAck(
  ...
  input: { messageId: string; runtimeId: string; timestamp: string }
): string {
  const payload = canonicalAckPayload(input);
  return sign(null, Buffer.from(payload), privateKeyPem).toString('base64');
}

export function signPendingPoll(
  privateKeyPem: string,
  input: { runtimeId: string; timestamp: string }
): string {
  const payload = canonicalPendingPollPayload(input);
  return sign(null, Buffer.from(payload), privateKeyPem).toString('base64');
}
