...
export async function startTestServer(): Promise<TestAtlasServer> {
  ...
  registerAtlasRoutes(router, { agentRepo, tokenRepo, accessTokenRepo, heartbeatRepo, syncRepo, activationTokenRepo });
  ...
  return { server, baseUrl, agentRepo, tokenRepo, accessTokenRepo, heartbeatRepo, syncRepo, activationTokenRepo };
}
...
export async function provisionAgent(
  ...
  rawToken: string,
  overrides: Partial<{
    machineId: string; hostname: string; connectorType: string;
    version: string; name: string; companyId: string;
  }> = {},
