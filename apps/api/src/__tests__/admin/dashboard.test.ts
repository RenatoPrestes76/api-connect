...
describe('GET /admin/dashboard/metrics', () => {
  ...
  it('counts last24h synchronizations from sync history', async () => {
    ...
    const sync = SyncRecord.create({ agentId: 'ag', startedAt: started, finishedAt: now, recordsSent: 10, recordsFailed: 0, bytesTransferred: 1024, result: 'SUCCESS' });
  ...
  it('response has all expected metric keys', async () => {
    ...
    expect(Object.keys(body.data)).toEqual(expect.arrayContaining(['companies','agents','online','stale','offline','last24hSynchronizations']));
  });
});
...
describe('GET /admin/dashboard/activity', () => {
  ...
  it('returns recent syncs', async () => {
    const now = new Date();
    const s   = SyncRecord.create({ agentId: 'ag', startedAt: new Date(now.getTime() - 1000), finishedAt: now, recordsSent: 5, recordsFailed: 0, bytesTransferred: 512, result: 'SUCCESS' });
  ...
  it('respects sinceMs query parameter', async () => {
    const old = HeartbeatRecord.create({ agentId: 'ag', receivedAt: new Date(Date.now() - 7_200_000), version: '1.0', hostname: 'h', status: 'OFFLINE' });
