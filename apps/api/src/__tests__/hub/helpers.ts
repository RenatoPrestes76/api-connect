...
export interface TestHubServer {
  server:  Server;
  baseUrl: string;
}

export async function startHubServer(): Promise<TestHubServer> {
...
export async function post<T>(baseUrl: string, path: string, payload?: unknown): Promise<{ status: number; body: T }> {
  const res  = await fetch(`${baseUrl}${path}`, {
    method:  'POST',
...
export async function put<T>(baseUrl: string, path: string, payload?: unknown): Promise<{ status: number; body: T }> {
  const res  = await fetch(`${baseUrl}${path}`, {
    method:  'PUT',
    headers: { 'Content-Type': 'application/json' },
    body:    payload != null ? JSON.stringify(payload) : undefined,
