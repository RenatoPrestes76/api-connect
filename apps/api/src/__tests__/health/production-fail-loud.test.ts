 ...
 */
/**
 * `docker rm -f` can return success to the CLI slightly before the
 * daemon finishes tearing the container down under load — confirmed
 * directly: an immediate `docker run` reusing the same name occasionally
 * hit "Conflict: name already in use" right after a preceding `rm -f`
 * reported success. A short settle delay avoids the race without masking
 * a real removal failure (the `rm -f` itself still isn't swallowed).
 */
...
d('ATLAS 46.32 — production fail-loud, real container', () => {
  ...
  it(
    'refuses to boot in NODE_ENV=production with required secrets missing — exits non-zero, never opens the port',
    async () =>
      withDockerLock(async () => {
        ...
        // than assume a fixed delay is long enough. Generous deadline for the
        // same reason as graceful-shutdown.test.ts: concurrent Docker load
        // from sibling test files sharing this one daemon can slow container
        // startup/inspection considerably.
  ...
  it(
    'boots and serves in NODE_ENV=production with a complete, valid secret set',
    async () =>
      withDockerLock(async () => {
        await removeContainer(CONTAINER);
        await execFileAsync('docker', [
