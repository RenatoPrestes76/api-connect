...
d('ATLAS 46.32 — graceful shutdown, real container, real SIGTERM', () => {
  ...
  it(
    'docker stop delivers a real SIGTERM; the process exits cleanly (code 0), well inside the 10s window',
    async () =>
      withDockerLock(async () => {
        ...
        // steady-state shutdown path. A generous deadline: under concurrent
        // load from sibling Docker-based test files sharing this one Docker
        // daemon, container startup can be considerably slower than in
        // isolation — confirmed directly (a 30s deadline flaked under full
        // suite load; this one hasn't).
        ...
        expect(
          healthy,
          `expected the container to report healthy before shutdown (last status: ${lastStatus})`
        ).toBe(true);
