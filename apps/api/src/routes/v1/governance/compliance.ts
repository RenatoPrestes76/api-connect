...
export function registerComplianceRoutes(router: Router): void {
  ...
  router.get('/api/v1/compliance/status', async (_ctx: RouteContext, res: ServerResponse) => {
    json(res, governanceStore.getComplianceStatus());
  });

  // GET /api/v1/compliance/evidence
  router.get('/api/v1/compliance/evidence', async (ctx: RouteContext, res: ServerResponse) => {
