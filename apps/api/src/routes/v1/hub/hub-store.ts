...
export interface HubSettings {
  sync:          { intervalMs: number; retryAttempts: number; timeoutMs: number; batchSize: number; enableIncremental: boolean };
  cache:         { ttlMs: number; maxEntries: number };
  discovery:     { autoRunOnConnect: boolean; confidenceMinimum: number; schemaTtlMs: number };
  notifications: { enableEmailAlerts: boolean; alertEmail: string; alertOnFailure: boolean; alertOnDegraded: boolean };
}
...
class HubStore {
  ...
  private _seed() {
    ...
    const agentA: AgentSummary = {
      id: AGENT_IDS[0]!,  hostname: 'prod-worker-01', version: '2.4.1',
      os: 'Linux x86_64', ip: '10.0.1.11', status: 'ONLINE',
      lastSeen: minutesAgo(1), connectors: 2, syncCount: 1_420, errorCount: 3,
    };
    const agentB: AgentSummary = {
      id: AGENT_IDS[1]!,  hostname: 'staging-worker-02', version: '2.4.0',
      os: 'Linux x86_64', ip: '10.0.1.22', status: 'STALE',
      lastSeen: minutesAgo(8), connectors: 1, syncCount: 388, errorCount: 12,
    };
    ...
    const conns: ConnectorInstance[] = [
      {
        id: CONN_IDS[0]!, name: 'ERP-Prod', version: '1.3.0', driver: 'postgresql',
        database: 'erp_prod', host: 'db.prod.internal', status: 'RUNNING',
        lastSync: minutesAgo(2), syncCount: 840, errorCount: 1,
        health: 'healthy', agentId: AGENT_IDS[0],
      },
      {
        id: CONN_IDS[1]!, name: 'CRM-Prod', version: '1.2.4', driver: 'mysql',
        database: 'crm', host: 'crm-db.prod.internal', status: 'RUNNING',
        lastSync: minutesAgo(5), syncCount: 580, errorCount: 2,
        health: 'healthy', agentId: AGENT_IDS[0],
      },
      {
        id: CONN_IDS[2]!, name: 'ERP-Staging', version: '1.3.0', driver: 'postgresql',
        database: 'erp_staging', host: 'db.staging.internal', status: 'ERROR',
        lastSync: minutesAgo(62), syncCount: 388, errorCount: 12,
        health: 'unhealthy', agentId: AGENT_IDS[1],
      },
    ];
    ...
    const dbs: DatabaseConnection[] = [
      {
        id: DB_IDS[0]!, name: 'ERP Production', driver: 'postgresql',
        host: 'db.prod.internal', port: 5432, database: 'erp_prod',
        version: 'PostgreSQL 15.3', status: 'healthy',
        latencyMs: 4, poolSize: 20, poolUsed: 8,
        connectedAt: minutesAgo(480), schema: 'public',
      },
      {
        id: DB_IDS[1]!, name: 'CRM Production', driver: 'mysql',
        host: 'crm-db.prod.internal', port: 3306, database: 'crm',
        version: 'MySQL 8.0.33', status: 'degraded',
        latencyMs: 28, poolSize: 10, poolUsed: 9,
        connectedAt: minutesAgo(480), schema: 'crm',
      },
    ];
    dbs.forEach((d) => this.databases.set(d.id, d));

    // Sync history (most recent first)
    const results: SyncResult[] = ['SUCCESS','SUCCESS','SUCCESS','PARTIAL','SUCCESS','FAILED','SUCCESS','SUCCESS'];
    for (let i = 0; i < 40; i++) {
      const connIdx = i % 3;
      ...
      this.syncHistory.push({
        id: randomUUID(), connectorId: cid, connector: cname,
        agentId: AGENT_IDS[connIdx % 2]!,
        startedAt: started, finishedAt: finished, durationMs: dur,
        result, synced: dur ? 50 + Math.floor(Math.random() * 500) : 0,
        skipped: Math.floor(Math.random() * 20), failed: result === 'FAILED' ? 5 : 0,
    ...
    // Users
    const users: HubUser[] = [
      { id: randomUUID(), name: 'Super Admin', email: 'admin@example.com', role: 'SUPER_ADMIN', active: true, createdAt: minutesAgo(43200), lastLogin: minutesAgo(30) },
      { id: randomUUID(), name: 'Operator',    email: 'ops@example.com',   role: 'OPERATOR',    active: true, createdAt: minutesAgo(21600), lastLogin: minutesAgo(120) },
      { id: randomUUID(), name: 'Viewer',      email: 'viewer@example.com', role: 'READ_ONLY',  active: true, createdAt: minutesAgo(10800), lastLogin: minutesAgo(720) },
    ];
  ...
  private _defaultSettings(): HubSettings {
    return {
      sync:          { intervalMs: 60_000, retryAttempts: 3, timeoutMs: 30_000, batchSize: 100, enableIncremental: true },
      cache:         { ttlMs: 1_800_000, maxEntries: 1_000 },
      discovery:     { autoRunOnConnect: true, confidenceMinimum: 60, schemaTtlMs: 3_600_000 },
      notifications: { enableEmailAlerts: false, alertEmail: '', alertOnFailure: true, alertOnDegraded: true },
    };
  }
}
