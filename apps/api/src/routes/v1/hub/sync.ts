...
export async function hubCancelSync(ctx: RouteContext, res: ServerResponse): Promise<void> {
  ...
  if (!record) { apiError(res, 'Sync record not found', 404, 'NOT_FOUND'); return; }
  if (record.result !== 'RUNNING') { apiError(res, 'Sync is not running', 409, 'INVALID_STATE'); return; }
...
export async function hubRetrySync(ctx: RouteContext, res: ServerResponse): Promise<void> {
  ...
  if (!original) { apiError(res, 'Sync record not found', 404, 'NOT_FOUND'); return; }
