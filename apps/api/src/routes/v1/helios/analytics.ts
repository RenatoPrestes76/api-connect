...
export function registerAnalyticsRoutes(router: Router): void {
  router.get('/api/v1/helios/analytics/stream', async (_ctx: RouteContext, res: ServerResponse) => {
    json(res, heliosStore.getStreamMetrics());
  });

  router.get('/api/v1/helios/analytics/topics', async (ctx: RouteContext, res: ServerResponse) => {
