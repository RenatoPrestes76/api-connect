...
import type { DiscoveryStatus } from '../../../modules/erp-metadata/types.js';
...
export function registerErpMetadataAdminRoutes(router: Router): void {
  ...
  router.get(
    ...
    requirePermission('erp-metadata.read')(async (ctx: RouteContext, res: ServerResponse) => {
      ...
      const status = (ctx.query.get('status') as DiscoveryStatus | null) ?? undefined;
      const requests = erpMetadataStore.listRequests({ runtimeId, organizationId, status });
      json(res, {
