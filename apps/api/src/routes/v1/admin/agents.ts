  ...
  return [...agents].sort((a, b) => {
    ...
    switch (sort.by) {
      ...
      case 'name':
        ...
        break;
      case 'lastHeartbeat':
        va = sa.lastHeartbeat?.getTime() ?? 0;
        vb = sb.lastHeartbeat?.getTime() ?? 0;
        break;
      case 'lastSynchronization':
        va = sa.lastSynchronization?.getTime() ?? 0;
        vb = sb.lastSynchronization?.getTime() ?? 0;
        break;
      default:
        va = sa.createdAt.getTime();
        vb = sb.createdAt.getTime();
    }
