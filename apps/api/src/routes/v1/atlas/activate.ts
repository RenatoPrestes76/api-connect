...
export function createActivateHandler(
  ...
  return async (ctx: RouteContext, res: ServerResponse): Promise<void> => {
    ...
    if (!activationToken || !name || !hostname || !machineId || !version) {
      apiError(res, 'activationToken, name, hostname, machineId and version are required', 422, 'VALIDATION_ERROR');
      return;
    }
    ...
    if (existing) {
      apiError(res, `Machine ${machineId} is already registered`, 409, 'MACHINE_ALREADY_REGISTERED');
      return;
    }

    // 3. Register the agent
    let agent: InstanceType<typeof AtlasAgent>;
    try {
