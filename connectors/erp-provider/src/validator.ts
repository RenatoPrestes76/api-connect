import type {
  ConnectorResult, ValidationResult, ValidationIssue, ConnectorContext,
...
export class ErpValidator {
  ...
  async validate(): Promise<ConnectorResult<ValidationResult>> {
    ...
    if (!(await this._ctx.credentials.has('username'))) {
      errors.push({ field: 'username', code: 'CREDENTIAL_MISSING', message: 'username credential not set' });
    }
    if (!(await this._ctx.credentials.has('password'))) {
      errors.push({ field: 'password', code: 'CREDENTIAL_MISSING', message: 'password credential not set' });
    }

    // 3. Timeout range advisory
    const timeout = this._ctx.config.getNumber('timeout', 5000) ?? 5000;
    if (timeout < 100) {
      warnings.push({ field: 'timeout', code: 'TIMEOUT_LOW', message: 'timeout below 100ms is not recommended' });
    } else if (timeout > 30_000) {
      warnings.push({ field: 'timeout', code: 'TIMEOUT_HIGH', message: 'timeout above 30s may cause sync delays' });
    }
