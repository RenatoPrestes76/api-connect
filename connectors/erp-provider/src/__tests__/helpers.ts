...
const MOCK_SCHEMA: DatabaseSchema = {
  ...
  tables: [
    ...
    {
      name: 'estoque',
      columns: [COL('cod_produto'), COL('cod_deposito'), COL('qtd_atual'), COL('qtd_reservada')],
      primaryKey: undefined,
      foreignKeys: [],
      indexes: [],
    },
    {
      name: 'pedidos',
      columns: [COL('numero'), COL('cod_cliente'), COL('status'), COL('valor_total')],
      primaryKey: undefined,
      foreignKeys: [],
      indexes: [],
    },
    {
      name: 'fornecedores',
      columns: [COL('codigo'), COL('razao_social'), COL('cnpj'), COL('ativo')],
      primaryKey: undefined,
      foreignKeys: [],
      indexes: [],
    },
    {
      name: 'usuarios',
      columns: [COL('login'), COL('nome'), COL('email'), COL('perfil'), COL('ativo')],
      primaryKey: undefined,
      foreignKeys: [],
      indexes: [],
    },
...
const MOCK_CUSTOMER_ROWS = [
  ...
  {
    codigo: 'CUST-003',
    razao_social: 'Initech LLC',
    email: 'initech@example.com',
    cnpj_cpf: '11.222.333/0001-00',
    limite_credito: 25000,
    ativo: false,
  },
  {
    codigo: 'CUST-004',
    razao_social: 'Umbrella Corp',
    email: 'umbrella@example.com',
    cnpj_cpf: '55.666.777/0001-00',
...
const MOCK_DB_HEALTH: DatabaseHealth = {
  connected: true,
  latency: 1,
  databaseVersion: 'PostgreSQL 15.0 (mock)',
  activeConnections: 1,
  poolUsage: 0.1,
  status: 'healthy',
};
