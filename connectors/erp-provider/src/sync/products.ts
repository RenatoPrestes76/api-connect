...
export class ProductsSync {
  ...
  async sync(context: SyncContext): Promise<ProductsSyncResult> {
    ...
    const tableName = resolveTable('products', schema.tables.map((t) => t.name));

    if (!tableName) {
      this._ctx.logger.warn('Products table not found — skipping', { candidates: PRODUCT_TABLE_CANDIDATES });
      return {
    ...
    this._ctx.logger.debug('Products sync complete', { synced: count, durationMs: Date.now() - start });
    return {
