...
describe('PostgresDialect', () => {
  ...
  it('renderLimit adds LIMIT + OFFSET', () =>
    expect(d.renderLimit(10, 20)).toBe(' LIMIT 10 OFFSET 20'));
  it('renderLimit only LIMIT when no offset', () => expect(d.renderLimit(5)).toBe(' LIMIT 5'));
  it('renderLimit only OFFSET when no limit', () =>
    expect(d.renderLimit(undefined, 10)).toBe(' OFFSET 10'));
...
describe('MySQLDialect', () => {
  ...
  it('placeholder always ?', () => {
    expect(d.placeholder(1)).toBe('?');
    expect(d.placeholder(99)).toBe('?');
  });
...
describe('SQLServerDialect', () => {
  ...
  it('renderLimit uses FETCH NEXT', () =>
    expect(d.renderLimit(10, 20)).toBe(' OFFSET 20 ROWS FETCH NEXT 10 ROWS ONLY'));
  it('renderLimit defaults offset to 0', () =>
    expect(d.renderLimit(5)).toBe(' OFFSET 0 ROWS FETCH NEXT 5 ROWS ONLY'));
...
describe('OracleDialect', () => {
  ...
  it('renderLimit with both', () =>
    expect(d.renderLimit(10, 5)).toBe(' OFFSET 5 ROWS FETCH FIRST 10 ROWS ONLY'));
