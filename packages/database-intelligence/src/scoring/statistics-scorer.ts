...
const ROW_COUNT_TIERS: readonly RowCountTier[] = [
  {
    ...
    signals: [['LOOKUP', 70], ['CONFIGURATION', 60], ['PERMISSION', 40]],
  },
  {
    min: 21, max: 500,
    label: 'small (21–500 rows)',
    signals: [['LOOKUP', 50], ['CATEGORY', 50], ['BRANCH', 40], ['USER', 30], ['CONFIGURATION', 30]],
  },
  {
    min: 501, max: 10_000,
    label: 'medium (501–10k rows)',
    signals: [['CATEGORY', 30], ['SUPPLIER', 40], ['USER', 30], ['PRODUCT', 20]],
  },
  {
    min: 10_001, max: 100_000,
    label: 'large (10k–100k rows)',
    signals: [['PRODUCT', 40], ['CUSTOMER', 40], ['SUPPLIER', 30]],
  },
  {
    min: 100_001, max: 1_000_000,
    label: 'very large (100k–1M rows)',
    signals: [['SALE', 50], ['MOVEMENT', 50], ['PRODUCT', 20], ['CUSTOMER', 20]],
  },
  {
    min: 1_000_001, max: Infinity,
    label: 'huge (> 1M rows)',
    signals: [['MOVEMENT', 60], ['LOG', 60], ['AUDIT', 50], ['FISCAL', 40], ['SALE', 40]],
  },
];
