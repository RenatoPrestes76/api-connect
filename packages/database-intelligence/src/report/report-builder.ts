...
export class ReportBuilder {
  ...
  private _detectRisks(
    ...
    const lowConf = classifications.filter(
      (c) => c.entity !== 'UNKNOWN' && c.confidence < 40,
    );
    if (lowConf.length > 5) {
      risks.push({
        level:       'MEDIUM',
    ...
    if (circular.length > 0) {
      const tables = [...new Set(circular.flatMap((r) => [`${r.fromSchema}.${r.fromTable}`, `${r.toSchema}.${r.toTable}`]))];
      risks.push({
        level:       'MEDIUM',
        category:    'RELATIONSHIPS',
        description: `${circular.length} circular reference(s) detected — may complicate synchronization`,
        tables,
        suggestion:  'Review circular dependencies and consider breaking cycles with soft deletes or nullable FKs',
      });
    }

    // PRODUCT tables with no price or inventory link
    const products = classifications.filter((c) => c.entity === 'PRODUCT' && c.confidence >= HIGH_CONFIDENCE);
  ...
  private _buildSuggestions(
    ...
    return suggestions.sort((a, b) => {
      ...
      const confA = classifications.find((c) => `${c.tableSchema}.${c.tableName}` === a.table)?.confidence ?? 0;
      const confB = classifications.find((c) => `${c.tableSchema}.${c.tableName}` === b.table)?.confidence ?? 0;
      return confB - confA;
    });
  }
