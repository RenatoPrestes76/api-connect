...
function detectCircularRefs(tables: readonly TableInput[]): Set<string> {
...
export class RelationshipAnalyzer {
  analyze(
    ...
    for (const table of tables) {
      ...
      results.push(
        makeRelationship(
          fk1.referencedSchema,
          fk1.referencedTable,
          fk1.referencedColumns[0] ?? 'id',
          fk2.referencedSchema,
          fk2.referencedTable,
          fk2.referencedColumns[0] ?? 'id',
          'MANY_TO_MANY',
          'N:N',
          `junction:${table.name}`,
          90,
        ...
        if (toKey === fromKey) {
          ...
          results.push(
            makeRelationship(
              ...
              [
                hierarchical
                  ? `Column "${fk.columns[0]}" suggests parent-child hierarchy`
                  : `Self-referencing FK`,
              ]
            )
          );
