...
import type {
  AIResult, AgentId, AITaskType, AIProviderId, SessionId,
...
export interface SchemaAnalystAgent extends AIAgent {
  ...
  classifyEntity(entityName: string, fields: string[], context: AIContext, session: AgentSession): Promise<AIResult<AIRecommendation>>;
  classifyField(fieldName: string, entityName: string, context: AIContext, session: AgentSession): Promise<AIResult<AIRecommendation>>;
...
export interface MappingAnalystAgent extends AIAgent {
  ...
  suggestMapping(sourceField: string, targetField: string, context: AIContext, session: AgentSession): Promise<AIResult<AIRecommendation>>;
  resolveConflict(conflict: MappingConflictInput, context: AIContext, session: AgentSession): Promise<AIResult<AIRecommendation>>;
  validateTransformation(transformation: string, context: AIContext, session: AgentSession): Promise<AIResult<AIRecommendation>>;
}
...
export interface ERPSpecialistAgent extends AIAgent {
  ...
  identifyModule(erpProfileId: string, context: AIContext, session: AgentSession): Promise<AIResult<AIRecommendation>>;
  analyzeConventions(context: AIContext, session: AgentSession): Promise<AIResult<AIRecommendation[]>>;
}
...
export interface PerformanceAnalystAgent extends AIAgent {
  ...
  detectBottleneck(context: AIContext, session: AgentSession): Promise<AIResult<AIRecommendation[]>>;
  recommendOptimization(area: string, context: AIContext, session: AgentSession): Promise<AIResult<AIRecommendation>>;
}
...
export interface SyncAnalystAgent extends AIAgent {
  ...
  diagnoseSlowSync(context: AIContext, session: AgentSession): Promise<AIResult<AIRecommendation[]>>;
}
...
export interface SecurityAnalystAgent extends AIAgent {
  ...
  classifyDataSensitivity(context: AIContext, session: AgentSession): Promise<AIResult<AIRecommendation[]>>;
...
export interface ChangeAnalystAgent extends AIAgent {
  ...
  assessImpact(change: SchemaChangeInput, context: AIContext, session: AgentSession): Promise<AIResult<AIRecommendation>>;
  planMigration(change: SchemaChangeInput, context: AIContext, session: AgentSession): Promise<AIResult<AIRecommendation>>;
}
...
export interface AIAgentOrchestrator {
   ...
   * Run a single agent
   */
  run(agentId: AgentId, context: AIContext, session: AgentSession): Promise<AIResult<AgentAnalysisResult>>;

  /**
   * Run multiple agents in parallel
   */
  runAll(agentIds: AgentId[], context: AIContext, session: AgentSession): Promise<AIResult<AgentAnalysisResult[]>>;

  /**
   * Run all agents suitable for a task type
   */
  runForTaskType(taskType: AITaskType, context: AIContext, session: AgentSession): Promise<AIResult<AgentAnalysisResult[]>>;

  /**
   * Run agents in a pipeline — output of each feeds the next
   */
  runPipeline(pipeline: AgentPipelineStep[], session: AgentSession): Promise<AIResult<AgentPipelineResult>>;
}
