...
export class TableStatisticsEngine {
  ...
  async profileTable(
    ...
    for (let i = 0; i < columns.length; i += batchSize) {
      ...
      const profiles = await Promise.all(
        batch.map((col) =>
          this._profileColumn(schema, table, col, sampleSize, actualRowsSampled)
            .catch((): ColumnProfile => this._emptyColumnProfile(col, actualRowsSampled)),
        ),
      );
  ...
  private async _profileColumn(
    ...
    return {
      ...
      distinctCount,
      distinctPercent: nonNullCount > 0 ? Math.round((distinctCount / nonNullCount) * 10_000) / 100 : 0,
      avgLength:      row.avg_length !== null ? Math.round(parseFloat(row.avg_length) * 100) / 100 : null,
