...
function makeHBRow(id = 'hb-1', agentId = 'agent-1') {
  return { id, agentId, receivedAt: NOW, version: '1.0.0', hostname: 'h', memoryUsage: null, uptime: null, queueSize: null, status: 'ONLINE' };
}
...
describe('PrismaHeartbeatRecordRepository', () => {
  ...
  it('save calls db.create with correct data', async () => {
    ...
    const r = HeartbeatRecord.create({ agentId: 'agent-1', receivedAt: NOW, version: '1.0.0', hostname: 'h', status: 'ONLINE' });
    await repo.save(r);
    expect(mockDb.create).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ agentId: 'agent-1' }) }));
  });
  ...
  it('findRecent passes gte filter', async () => {
    ...
    expect(mockDb.findMany).toHaveBeenCalledWith(expect.objectContaining({ where: expect.objectContaining({ receivedAt: { gte: AGO } }) }));
  });
...
function makeSyncRow(id = 's-1', agentId = 'agent-1') {
  return { id, agentId, startedAt: AGO, finishedAt: NOW, durationMs: 3_600_000, recordsSent: 100, recordsFailed: 0, bytesTransferred: 1024, compressionRatio: null, result: 'SUCCESS' };
}
...
describe('PrismaSyncRecordRepository', () => {
  ...
  it('save calls db.create with correct data', async () => {
    ...
    const r = SyncRecord.create({ agentId: 'agent-1', startedAt: AGO, finishedAt: NOW, recordsSent: 100, recordsFailed: 0, bytesTransferred: 1024, result: 'SUCCESS' });
    await repo.save(r);
    expect(mockDb.create).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ agentId: 'agent-1' }) }));
  });
  ...
  it('findRecent passes gte filter on finishedAt', async () => {
    vi.mocked(mockDb.findMany).mockResolvedValue([]);
    await repo.findRecent(AGO);
    expect(mockDb.findMany).toHaveBeenCalledWith(expect.objectContaining({ where: expect.objectContaining({ finishedAt: { gte: AGO } }) }));
  });

  it('deleteOldest calls deleteMany for excess rows', async () => {
    vi.mocked(mockDb.findMany).mockResolvedValue([makeSyncRow('1'), makeSyncRow('2'), makeSyncRow('3')]);
    vi.mocked(mockDb.deleteMany).mockResolvedValue({ count: 1 });
    await repo.deleteOldest('agent-1', 2);
    expect(mockDb.deleteMany).toHaveBeenCalledWith({ where: { id: { in: ['3'] } } });
  });
