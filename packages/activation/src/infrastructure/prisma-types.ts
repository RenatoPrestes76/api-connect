...
export interface ActivationTokenDbDelegate {
  create(args: {
    data: Omit<PrismaActivationToken, 'createdAt'> & { createdAt?: Date };
  }): Promise<PrismaActivationToken>;
  upsert(args: {
    where: { id: string };
    create: PrismaActivationToken;
    update: Partial<PrismaActivationToken>;
  }): Promise<PrismaActivationToken>;
  findUnique(args: {
    where: { id?: string; token?: string };
  }): Promise<PrismaActivationToken | null>;
