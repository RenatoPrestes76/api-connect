...
function makePipeline(config = BASE_CONFIG, dispatcher?: CloudDispatcher, validator?: ValidationEngine) {
...
describe('SyncPipeline', () => {
  it('processes a basic batch and returns dispatched record count', async () => {
    ...
    const input: ExtractInput = {
      records: [{ id: 1, nome: 'Produto A' }, { id: 2, nome: 'Produto B' }],
      schema:  'public',
      table:   'produto',
    };
  ...
  it('casts Date values to ISO strings during transform', async () => {
    ...
    const dispatcher = {
      dispatch: vi.fn().mockImplementation(async (batch: { payload: Buffer }) => {
        ...
        return { ok: true, value: { accepted: 1, rejected: 0, batchId: 'b', latencyMs: 0, serverRef: 's' } };
      }),
    } as unknown as CloudDispatcher;
  ...
  it('encrypts payload when encryptionKey is provided', async () => {
    ...
    const pipeline = makePipeline(
      { ...BASE_CONFIG, encryption: { enabled: true, algorithm: 'aes-256-gcm' as const, keyId: 'key-1' } },
      dispatcher,
    );
