...
describe('RetryEngine', () => {
  ...
  it('notifies observer on each attempt', async () => {
    ...
    const fn = vi.fn().mockRejectedValueOnce(new Error('NETWORK_ERROR')).mockResolvedValue('done');
  ...
  it('calculates exponential backoff', () => {
    ...
    const calc = (
      engine as unknown as { _calculateDelay: (n: number) => number }
    )._calculateDelay.bind(engine);
