...
describe('CheckpointManager', () => {
  ...
  describe('updateTable()', () => {
    it('updates table status to IN_PROGRESS', async () => {
      ...
      const result = await manager.updateTable(JOB_ID, 'public', 'produto', {
        status: 'IN_PROGRESS',
        lastOffset: 1000,
      });
      expect(result.ok).toBe(true);
    ...
    it('returns error for unknown table', async () => {
      ...
      const result = await manager.updateTable(JOB_ID, 'public', 'nao_existe', {
        status: 'COMPLETED',
      });
  ...
  describe('isTableComplete()', () => {
    it('returns true for completed table', async () => {
      const { value: cp } = (await manager.create({
        jobId: JOB_ID,
        tenantId: TENANT,
        correlationId: CORR,
        mode: 'FULL',
        tables: TABLES,
      })) as { value: { tables: Map<string, { status: string }> } & object; ok: true };
      await manager.updateTable(JOB_ID, 'public', 'produto', { status: 'COMPLETED' });
      const updated = await manager.load(JOB_ID);
      expect(
        updated.ok && updated.value && manager.isTableComplete(updated.value, 'public', 'produto')
      ).toBe(true);
    });
