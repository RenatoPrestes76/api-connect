...
const TABLES = [
  { schema: 'public', table: 'produto',  rowCount: 1000, checksum: 'abc123', snapshotAt: new Date().toISOString() },
  { schema: 'public', table: 'cliente',  rowCount:  500, checksum: 'def456', snapshotAt: new Date().toISOString() },
];

describe('SnapshotManager', () => {
  it('creates a snapshot with generated id and totalRows', () => {
    const mgr = new SnapshotManager();
    const snap = mgr.create({ jobId: JOB, tenantId: TENANT, correlationId: CORR, database: 'erp', host: 'db.local', tables: TABLES });
  ...
  it('latest() returns most recent snapshot for tenant+database', () => {
    const mgr = new SnapshotManager();
    mgr.create({ jobId: JOB, tenantId: TENANT, correlationId: CORR, database: 'erp', host: 'h', tables: TABLES, note: 'first' });
    const second = mgr.create({ jobId: JOB, tenantId: TENANT, correlationId: CORR, database: 'erp', host: 'h', tables: TABLES, note: 'second' });
  ...
  it('listByTenant() returns only snapshots for that tenant', () => {
    ...
    mgr.create({ jobId: JOB, tenantId: TENANT,      correlationId: CORR, database: 'erp', host: 'h', tables: TABLES });
    mgr.create({ jobId: JOB, tenantId: TENANT,      correlationId: CORR, database: 'crm', host: 'h', tables: TABLES });
    mgr.create({ jobId: JOB, tenantId: otherTenant, correlationId: CORR, database: 'erp', host: 'h', tables: TABLES });
  ...
  it('delete() removes a snapshot', () => {
    const mgr = new SnapshotManager();
    const snap = mgr.create({ jobId: JOB, tenantId: TENANT, correlationId: CORR, database: 'erp', host: 'h', tables: TABLES });
  ...
  it('diff() detects changed tables', () => {
    const mgr   = new SnapshotManager();
    const before = mgr.create({ jobId: JOB, tenantId: TENANT, correlationId: CORR, database: 'erp', host: 'h', tables: TABLES });
    const after  = mgr.create({
      jobId: JOB, tenantId: TENANT, correlationId: CORR, database: 'erp', host: 'h',
      tables: [
