...
export class SyncPipeline {
  ...
  async processBatch(
    ...
    if (this._config.encryption.enabled && context.encryptionKey) {
      const encResult = await this._encryptor.encrypt(compressedBatch.payload, context.encryptionKey);
      if (!encResult.ok) {
    ...
    // ─── Stage 7: Dispatch ────────────────────────────────────────────────
    const dispatchResult = await this._dispatcher.dispatch(dispatchPayload, context.dispatchOptions);
  ...
  private _stageValidate(
    ...
    const results = this._validator.validateBatch(schema, table, records.map((r) => r.data));
