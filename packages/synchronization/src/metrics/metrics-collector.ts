...
export class MetricsCollector {
  ...
  batchDispatched(schema: string, table: string, originalBytes: number, compressedBytes: number): void {
    const acc = this._tables.get(`${schema}.${table}`);
  ...
  setTotalRecords(n: number): void { this._totalRecords = n; }
  retryOccurred():     void { this._retryCount++; }
  conflictDetected():  void { this._conflictsDetected++; }
  conflictResolved():  void { this._conflictsResolved++; }
  complete():          void { this._completedAt = Date.now(); this._status = 'COMPLETED'; }
  fail():              void { this._status = 'FAILED'; }
  pause():             void { this._status = 'PAUSED'; }
  resume():            void { this._status = 'RUNNING'; }
  ...
  metrics(): SyncMetrics {
    ...
    const compressionRatio = this._bytesProcessed > 0 ? this._bytesCompressed / this._bytesProcessed : 1;
  ...
  private _sampleMemory(): void {
    ...
    } catch { /* process may not be available in all environments */ }
  }
}
